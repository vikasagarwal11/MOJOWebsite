 ...
 * Run with: node scripts/check-stuck-files.js
 */
...
if (!admin.apps.length) {
  // Try to use service account from functions directory or environment
  try {
...
async function checkStuckFiles() {
  try {
    ...
    // Get all media files stuck in processing for more than 30 minutes
    const now = new Date();
    ...
    for (const doc of stuckQuery.docs) {
      ...
      const hoursStuck = (now.getTime() - createdAt.getTime()) / (1000 * 60 * 60);
    ...
    if (filesToFix.length > 0) {
      ...
      for (const file of filesToFix) {
        ...
        console.log(`   Fixing ${file.id}: ${file.transcodeStatus} → ${newStatus}`);
