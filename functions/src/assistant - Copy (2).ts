...
const DEFAULT_SIMILARITY_THRESHOLD = 0.35; // More lenient base threshold (was 0.22)
const BRAND_KB_THRESHOLD = 0.5; // More lenient for brand-specific questions (was 0.4)
const GENERIC_KB_THRESHOLD = 0.35; // More lenient for generic questions (was 0.22)
const SAFETY_NET_THRESHOLD = 0.5; // Safety net for brand questions with missing distances (was 0.7)
...
export const chatAsk = onCall(
  ...
  async request => {
    console.log('=== chatAsk v2.4: KB-First Strategy - All Questions Try KB First ===');
    try {
      ...
      // Phase 2: Brand detection (for threshold tuning only - KB-first strategy)
      // Note: Brand detection is now used only to adjust similarity thresholds, not to route questions
      // All questions proceed to KB search first, then fallback to general knowledge if needed
      const brandQuestion = isBrandQuestion(question);
      console.log(`[assistant.chatAsk] Question: "${question}" -> Brand question: ${brandQuestion} (used for threshold tuning)`);
      console.log('[assistant.chatAsk] Proceeding to KB search (KB-first strategy)');
      
      // Phase 3: KB search - Try KB search for all questions
      ...
      if (embedding) {
        ...
        try {
          ...
          // Dynamic threshold: lenient for brand questions, more permissive for all questions
          // Brand questions get 0.5 threshold (more lenient) since KB is focused on MFM content
          // Generic questions get 0.35 (more permissive) - KB-first strategy
          const config = await getAssistantConfig();
          const baseThreshold = config.similarityThreshold ?? DEFAULT_SIMILARITY_THRESHOLD;
          const SIMILARITY_THRESHOLD = brandQuestion 
            ? Math.max(baseThreshold, BRAND_KB_THRESHOLD) // Use 0.5 for brand, or config if higher
            : Math.max(baseThreshold, GENERIC_KB_THRESHOLD); // Use 0.35 for generic, or config if higher
          ...
          const isConfidentMatch = goodMatches.length >= minMatches && bestDistance !== undefined && bestDistance < SIMILARITY_THRESHOLD;
          
          // Safety net: if any question has docs but distances are missing, use KB anyway
          // This prevents silent fallback to general knowledge due to extraction issues
          const hasDocsButNoDistances = docs.length > 0 && allDistances.length === 0;
          // Safety net threshold: use KB if docs exist even with poor distances (for brand questions)
          // This ensures brand questions get KB answers even if embeddings aren't perfect
          // Lowered from 0.7 to 0.5 to avoid very poor matches while still being lenient
          
          // Log basic info
          console.log(`[assistant.chatAsk] KB Search Results: "${question}"`);
          console.log(`[assistant.chatAsk] - Is Brand Question: ${brandQuestion}`);
          ...
          console.log(`[assistant.chatAsk] - Confident Match: ${isConfidentMatch}`);
          ...
          if (hasDocsButNoDistances) {
            console.warn('[assistant.chatAsk] ⚠️ Docs present but no distances extracted; using KB as safety net');
          }
          ...
          if (docs.length === 0) {
            // No KB docs found - skip KB entirely
            shouldUseKB = false;
            ...
            // Docs exist but distances missing - use KB as safety net (extraction issue)
            shouldUseKB = true;
          ...
          console.log(`[assistant.chatAsk] 🎯 KB Decision:`);
          ...
          console.log(`[assistant.chatAsk]   - isBrandQuestion: ${brandQuestion}`);
          ...
          if (!docs.length) {
            ...
            const reason = !bestDistance && docs.length > 0
              ? 'Distance not available from vector search - distances too high or extraction issue'
              : bestDistance !== undefined && bestDistance >= 0.6
                ? `KB results too poor (distance: ${bestDistance.toFixed(3)} >= 0.6, skipping KB)`
                : bestDistance !== undefined && bestDistance >= SIMILARITY_THRESHOLD
                  ? `KB results not confident enough (distance: ${bestDistance.toFixed(3)}, threshold: ${SIMILARITY_THRESHOLD})`
                  : `Not enough good matches found (${goodMatches.length} match(es), need at least ${minMatches})`;
            ...
            if (isConfidentMatch) {
              console.log(`[assistant.chatAsk] ✅ Using KB with confident match (distance: ${bestDistance?.toFixed(3) ?? 'N/A'})`);
            } else if (bestDistance !== undefined && bestDistance < SAFETY_NET_THRESHOLD) {
              console.log(`[assistant.chatAsk] ✅ Using KB with medium confidence (distance: ${bestDistance.toFixed(3)} < ${SAFETY_NET_THRESHOLD})`);
            } else if (hasDocsButNoDistances) {
              console.log('[assistant.chatAsk] ✅ Using KB as safety net (docs exist but distances missing)');
            } else if (brandQuestion && bestDistance !== undefined && bestDistance < 0.6) {
              console.log(`[assistant.chatAsk] ✅ Using KB for brand question (lenient threshold: distance ${bestDistance.toFixed(3)} < 0.6)`);
            }
          }
        } catch (error: any) {
