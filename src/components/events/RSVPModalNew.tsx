...
export const RSVPModalNew: React.FC<RSVPModalProps> = ({ 
  ...
  const [bulkFormData, setBulkFormData] = useState({
    familyMembers: [
      { name: '', ageGroup: '11+' as AgeGroup, relationship: 'guest' as Relationship, rsvpStatus: 'going' as AttendeeStatus }
    ]
  });
  ...
  // Get family members that haven't been added to this event yet
  const availableFamilyMembers = familyMembers.filter(familyMember => 
    !attendees.some(attendee => 
      attendee.userId === currentUser?.id && 
      attendee.name === familyMember.name &&
      attendee.attendeeType === 'family_member'
    )
  );

  // Check if event is past
  const isEventPast = event.startAt && new Date(event.startAt) < new Date();
  ...
  const handleBulkAddFamilyMembers = async () => {
    ...
    try {
      ...
      // Reset form
      setBulkFormData({
        familyMembers: [{ name: '', ageGroup: '11+', relationship: 'guest', rsvpStatus: 'going' }]
      });
  ...
  const addBulkFormRow = () => {
    setBulkFormData(prev => ({
      familyMembers: [...prev.familyMembers, 
        { name: '', ageGroup: '11+', relationship: 'guest', rsvpStatus: 'going' }
      ]
    }));
  };
  ...
  const updateBulkFormField = (index: number, field: keyof typeof bulkFormData.familyMembers[0], value: string) => {
    setBulkFormData(prev => ({
            ...
            <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold">RSVP for {event.title}</h2>
                    <p className="text-purple-100 text-sm">
                      Manage your attendance and family members
                    </p>
                  </div>
                </div>
                <motion.button
                                ...
                                {bulkFormData.familyMembers.map((member, index) => (
                                  <div key={index} className="grid grid-cols-1 sm:grid-cols-5 gap-2 items-center">
                                    <input
                                      ...
                                      placeholder="Name"
                                      value={member.name}
                                      onChange={(e) => updateBulkFormField(index, 'name', e.target.value)}
                                      className="px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    />
                                    
                                    <select
                                      value={member.ageGroup}
                                      onChange={(e) => updateBulkFormField(index, 'ageGroup', e.target.value)}
                                      className="px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    >
                                    ...
                                    <select
                                      ...
                                      onChange={(e) => updateBulkFormField(index, 'relationship', e.target.value)}
                                      className="px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    >
                                    ...
                                    <select
                                      ...
                                      onChange={(e) => updateBulkFormField(index, 'rsvpStatus', e.target.value)}
                                      className="px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    >
