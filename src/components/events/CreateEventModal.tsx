...
import { computeGeohash } from '../../services/geoService';
...
const CreateEventModal: React.FC<CreateEventModalProps> = ({ onClose, onEventCreated, eventToEdit }) => {
  ...
  useEffect(() => {
    if (eventToEdit) {
      ...
      // Initialize geo state from existing event
      setGeoLatitude(eventToEdit.latitude ?? null);
      setGeoLongitude(eventToEdit.longitude ?? null);
      setGeoHash(eventToEdit.geohash ?? null);
      setGeoRegion(eventToEdit.region ?? null);
    }
  ...
  const selectAddress = (suggestion: any) => {
    ...
    // Capture geo data from Nominatim response
    try {
      ...
      if (!isNaN(lat) && !isNaN(lon)) {
        setGeoLatitude(lat);
        setGeoLongitude(lon);
        setGeoHash(computeGeohash(lat, lon));
        // Extract region from the address (state abbreviation from secondary text)
        const addressParts = suggestion.description?.split(',') || [];
        const statePart = addressParts.length >= 3 ? addressParts[addressParts.length - 2]?.trim() : null;
        setGeoRegion(statePart || null);
      }
    } catch (err) {
      console.warn('Failed to compute geo data from address:', err);
      setGeoLatitude(null);
      setGeoLongitude(null);
      setGeoHash(null);
      setGeoRegion(null);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.length > 0) {
          ...
          // Capture geo data from venue resolution
          try {
            ...
            if (!isNaN(lat) && !isNaN(lon)) {
              ...
              setGeoLongitude(lon);
              setGeoHash(computeGeohash(lat, lon));
              const addressParts = venue.display_name?.split(',') || [];
              const statePart = addressParts.length >= 3 ? addressParts[addressParts.length - 2]?.trim() : null;
              setGeoRegion(statePart || null);
            }
          } catch (err) {
            console.warn('Failed to compute geo data from venue resolution:', err);
            setGeoLatitude(null);
            setGeoLongitude(null);
            setGeoHash(null);
            setGeoRegion(null);
  ...
  const selectVenue = (venue: any) => {
    ...
    // Capture geo data from venue selection
    try {
      const lat = parseFloat(venue.lat);
      const lon = parseFloat(venue.lon);
      if (!isNaN(lat) && !isNaN(lon)) {
      ...
      console.warn('Failed to compute geo data from venue selection:', err);
      setGeoLatitude(null);
      setGeoLongitude(null);
      setGeoHash(null);
      setGeoRegion(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6">
      ...
      <motion.div
        ...
        className={`relative flex max-h-[90vh] w-full flex-col overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/5 ${previewMode ? 'max-w-6xl' : 'max-w-2xl'
          }`}
      >
        {/* Header */}
        <div className="flex items-center justify-between gap-3 bg-gradient-to-r from-[#F25129] to-[#FFC107] px-4 py-3 text-white sm:px-6 sm:py-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/30 bg-white/10 shadow-sm transition hover:bg-white/20 ${previewMode ? 'ring-2 ring-white/30' : ''
                }`}
                          ...
                          {searchResults.map((user) => (
                            <div key={user.id} className="flex items-center justify-between p-2 bg-blue-50 rounded-lg border border-blue-200">
                              ...
                              <button
                                ...
                                className={`px-3 py-1 text-xs rounded transition-colors ${invitedUserIds.includes(user.id)
                ...
                <div className="space-y-4">
                  ...
                  {requiresPayment && (
                    ...
                    >
                      ...
                      {!payThere && (
                        <div className={`p-3 rounded-lg border ${paymentMethod === 'stripe'
