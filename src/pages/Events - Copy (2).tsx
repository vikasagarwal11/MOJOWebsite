...
const calendarTooltipStyles = `
  ...
  /* Let React Big Calendar handle overflow naturally */

 /* Month view: compact chips only */
...
/* Time views (week/day): style only, no fixed height */
.rbc-time-view .rbc-event {
  ...
  .rbc-month-row {
    ...
    /* Let React Big Calendar handle all layout naturally */

  /* Let React Big Calendar handle column layout naturally */
  ...
  /* Let React Big Calendar handle week view styling naturally */
  ...
  /* Let React Big Calendar handle "Today" indicators naturally */

  /* Let React Big Calendar handle today's cell layout naturally */
`;
...
const Events: React.FC = () => {
  ...
  const { events, upcomingEvents, pastEvents, loading: eventsLoading, error: eventsError } = useEvents();
  ...
  // Events are now loaded via useEvents hook
  // useEffect removed - events now loaded via useEvents hook

    // Past events logic removed - now handled by useEvents hook

    // Upcoming events logic removed - now handled by useEvents hook
    if (currentUser) {
  ...
  const filteredEvents = useMemo(() => {
    ...
    if (activeTab === 'upcoming') {
      console.log('🔍 Filtered Events Debug:', {
        ...
        events: filtered.map(e => ({ id: e.id, title: e.title, startAt: e.startAt }))
      });
    }
    
    return filtered;
  }, [debouncedSearchQuery, tagFilter, activeTab, upcomingEvents, pastEvents, currentUser]);
  ...
  const calendarEvents = useMemo(() => {
    ...
    return uniqueEvents.map((e: AnyEvent) => {
       ...
       let endTime: Date;
               ...
               // Handle end time properly with better validation
        if (e.endAt) {
          endTime = tsToDate(e.endAt);
          // Validate that endAt is actually after startAt
          if (endTime <= startTime) {
            console.warn(`⚠️ Event "${e.title}" has endAt (${endTime.toLocaleString()}) <= startAt (${startTime.toLocaleString()}). Using duration fallback.`);
        ...
        console.log(`📅 Event: ${e.title}`, {
          ...
          originalEndAt: e.endAt,
          originalDuration: e.duration,
          startTimestamp: e.startAt,
          endTimestamp: e.endAt
        });
          ...
          allDay: e.allDay || false, // Use the allDay field from the event
          resource: e,
        };
     });
  }, [filteredEvents, currentUser, activeTab]);
    ...
    return (
      <>
        ...
        <div className="rounded-xl border bg-white p-6">
          ...
          {!teaserList.length && !publicList.length && (
            <div className="text-center py-8">
              <Calendar className="w-12 h-12 text-gray-300 mx-auto mb-2" />
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 relative overflow-visible">
           ...
           onSelectEvent={(event: any) => {
            ...
            defaultView="month"
              ...
              return {
                className: isTeaser ? 'bg-gray-400 opacity-50' : isPublic ? 'bg-purple-600' : 'bg-red-500',
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {viewMode === 'grid'
        ...
        : renderCalendar()}
      {/* Error banner */}
      {eventsError && <div className="mt-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-red-700">{eventsError}</div>}
      {/* Create Event Modal */}
      {isCreateModalOpen && (
        <CreateEventModal
          onClose={() => {
