...
class MediaService {
  ...
  Future<String> uploadStoryFile(File file, String type) async {
    ...
    final useExt = _isVideo(normalized) ? (ext == 'mov' || ext == 'webm' ? ext : 'mp4') : ext;
    final path = 'stories/${user.uid}/$id.$useExt';

    final ref = _storage.ref().child(path);
    final meta = SettableMetadata(contentType: _contentType(useExt, video: _isVideo(normalized)));
  ...
  Future<String> uploadFeedMedia(File file, String type) async {
    final user = _auth.currentUser;
    if (user == null) throw Exception('User not logged in');
    ...
    final ref = _storage.ref().child(path);
    final meta = SettableMetadata(contentType: _contentType(ext, video: _isVideo(normalized)));
    final snap = await ref.putFile(file, meta);
