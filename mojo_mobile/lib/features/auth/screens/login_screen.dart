...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  Future<void> _sendVerificationCode() async {
    ...
    if (!firebaseOptionsConfigured) {
      _showError(
          'Firebase is not configured. Run flutterfire configure in mojo_mobile.');
      return;
    }
    ...
    try {
      ...
      if (!registered) {
        _showError(
            'This phone number is not registered. Tap Create Account to join MOJO.');
        return;
      }
      ...
      await auth.startPhoneVerification(
        ...
        verificationCompleted: (credential) async {
          ...
          setState(() => _loading = true);
          try {
            await auth.signInWithPhoneCredential(credential,
  ...
  Future<void> _submitCode() async {
    ...
    try {
      ...
      if (e.code == 'user-doc-missing' ||
          (e.message?.contains('No account found') ?? false)) {
  ...
  Future<void> _routeAfterSuccessfulLogin(
      BuildContext context, WidgetRef ref) async {
    final uid = ref.read(authStateProvider).valueOrNull?.uid;
    if (uid == null || !context.mounted) return;
    final status =
        await ref.read(authServiceProvider).getUserProfileStatus(uid);
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (_step == _LoginStep.phone)
                      Text(
                        'By signing in, I consent to receive SMS and call notifications for verification, login, Event and account updates at this phone number.',
                        style: TextStyle(
