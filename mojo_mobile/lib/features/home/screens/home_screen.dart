...
import 'package:mojo_mobile/features/home/widgets/stories_bar.dart';
import 'package:mojo_mobile/features/events/widgets/rsvp_bottom_sheet.dart';
import 'package:mojo_mobile/core/widgets/app_loading_indicator.dart';

/// Keeps [ref.listen] out of a conditional â€” Riverpod requires a stable listener lifecycle.
              ...
              Consumer(
                builder: (context, ref, _) {
                  ...
                  return IconButton(
                    ...
                    onPressed: () {
                      final profile =
                          ref.read(userProfileProvider(user.uid)).valueOrNull;
                      final name =
                      ...
                      showDialog<void>(
                        context: context,
                        builder: (ctx) =>
                            QrInviteDialog(userId: user.uid, userName: name),
                      );
                    },
                  );
                },
...
class _WelcomeSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final user = ref.watch(authStateProvider).valueOrNull;
    final uid = user?.uid;
    final profileName = uid != null
        ? ref.watch(userProfileProvider(uid)).valueOrNull?.resolvedPublicName
        : null;
    final firstName = (profileName ??
            user?.displayName ??
        ...
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Ready for today, $firstName?',
                    style:
                        const TextStyle(color: Colors.white70, fontSize: 16)),
                const Text('Let\'s crush it! ðŸ’ª',
                    style: TextStyle(
                        color: Colors.white,
                        fontSize: 24,
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return _HomeEventCard(
                event: ev,
                onOpenDetail: () => context.push('/events/${ev.id}/detail'),
          ...
          itemBuilder: (context, index) {
            ...
            return ListTile(
              ...
              leading: CircleAvatar(
                backgroundImage: p.authorPhotoUrl != null
